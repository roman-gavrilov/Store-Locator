import {
  ...
  export default function HomePage() {
    ...
    useEffect(() => {
      ...
      authenticatedFetch(`/api/locations/${params.id}`).then((res) => res.json()).then((res) => {
        res.map((r) => {
          return r.pop();
        })
        setLocations(res);
              ...
              { locations && locations.length == 0 ?
                ...
                <div className="locations">
                  ...
                  <Pagination
                      hasPrevious
                      onPrevious={() => {
                        if(parseInt(params.id) - 1 > 0) {
                          navigate(`/${parseInt(params.id) - 1}`);
                          setLoading(true);
                          authenticatedFetch(`/api/locations/${parseInt(params.id) - 1}`).then((res) => res.json()).then((res) => {
                            res.map((r) => {
                              return r.pop();
                            })
                            setLocations(res);
